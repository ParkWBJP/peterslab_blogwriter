...
const MAX_DRAFT_EXPANSION_ATTEMPTS = 2;
...
function draftMeetsMinimumLength(payload) {
  const safeVersion = payload?.safeVersion;
  if (!safeVersion) {
    return false;
  }

  return ["ja", "ko"].every((language) =>
...
function buildEmojiUsageInstruction(mode) {
  if (mode === "light") {
    return "Use a small number of gentle emojis in the body only. Keep them sparse, premium, and natural. Do not use emojis in every paragraph.";
  }

  if (mode === "warm") {
    return "Use a moderate number of friendly emojis in the body only. Keep them readable, warm, and still premium. Avoid excessive repetition or childish styling.";
  }
...
function buildDraftPrompt(body) {
  ...
  return [
    ...
    `For both aiOriginal and safeVersion, the Japanese and Korean article text must each be at least ${MIN_PRIMARY_ARTICLE_CHAR_COUNT} characters total when counting title + lead + body + CTA + meta description.`,
    ...
    "Japanese and Korean must both satisfy the minimum length. English can stay concise.",
...
function buildDraftExpansionPrompt(body, draftPayload) {
  ...
  return [
    ...
    `Japanese and Korean safeVersion text must each be at least ${MIN_PRIMARY_ARTICLE_CHAR_COUNT} characters total when counting title + lead + body + CTA + meta description.`,
    "Count visible Unicode characters, not bytes.",
...
async function createDraftResponse(body) {
  ...
  let payload = coerceDraftPayload(parseJsonObject(extractOutputText(draftResponse)));

  for (let attempt = 0; attempt < MAX_DRAFT_EXPANSION_ATTEMPTS && !draftMeetsMinimumLength(payload); attempt += 1) {
    const expandedResponse = await callOpenAI({
      ...
      instructions:
        "You are PetersLab's multilingual drafting and compliance-support assistant. Expand the draft so Japanese and Korean both satisfy the minimum character requirement while staying natural and compliant.",
      input: buildDraftExpansionPrompt(body, payload),
    });

    payload = coerceDraftPayload(parseJsonObject(extractOutputText(expandedResponse)));
  }

  if (!draftMeetsMinimumLength(payload)) {
